...
export function sampleWave(point: WavePoint, time: number, pointer: WavePointer) {
  const phase = point.x * 0.014 + point.y * 0.011 + time * 0.0042;
  const cross = Math.cos(point.y * 0.018 - time * 0.0031);
  const sweep = Math.sin(point.x * 0.008 - time * 0.0036);
  ...
  const travellingBand = Math.sin(point.col * 0.32 + point.row * 0.1 - time * 0.011);
  const bandLift = Math.max(0, travellingBand - 0.72) / 0.28;

  return {
    x: point.x + Math.sin(point.y * 0.013 + time * 0.0034) * 18 + pushX,
    y: point.y + depth * 48 - bandLift * 26 - influence * 68 + pushY,
    radius: Math.max(1.45, 2.2 + depth * 0.9 + bandLift * 1.1 + influence * 1.3),
